    ...
    <div class="col-md-12">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <form class="form-horizontal" method="post" action="<?= base_url('kpi_reports/dashboard_packer') ?>" id="form-filter-kpi-packer">
...
<div class="row">
    <div class="col-lg-6">
        <div class="panel panel-default">
            <div class="panel-heading">
                <i class="fa fa-bar-chart"></i> Hourly Packing Performance
            </div>
            <div class="panel-body">
    ...
    <div class="col-lg-12">
        <div class="panel panel-warning">
            <div class="panel-heading" style="background-color: #f0ad4e; color: white;">
                <i class="fa fa-trophy"></i> <strong>TIM INTI - Packer Performance</strong>
            </div>
            <div class="panel-body">
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            <?php if (isset($dashboard_stats['top_packers_inti']) && count($dashboard_stats['top_packers_inti']) > 0): ?>
                                <?php foreach ($dashboard_stats['top_packers_inti'] as $index => $packer): ?>
                                    <?php 
                                        // Determine performance status based on SKU Diversity Index
                                        $avg_sku = $packer['avg_sku_per_resi'] ?? 0;
                                        $total_qty = $packer['total_qty'] ?? 0;
                                        $total_sku_unique = $packer['total_sku_unique'] ?? 0;
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-right"><?= number_format($packer['total_sku_unique']) ?></td>
                                        <td class="text-right"><?= number_format($packer['total_qty']) ?></td>
                                        <td class="text-right"><?= number_format($packer['avg_sku_per_resi'], 2) ?></td>
    ...
    <div class="col-lg-12">
        <div class="panel panel-default">
            <div class="panel-heading" style="background-color: #e0e0e0;">
                <i class="fa fa-users"></i> <strong>TIM OTHERS - User Lain yang Bantu Packing</strong>
            </div>
            <div class="panel-body">
                <div class="table-responsive">
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            <?php if (isset($dashboard_stats['top_packers_others']) && count($dashboard_stats['top_packers_others']) > 0): ?>
                                <?php foreach ($dashboard_stats['top_packers_others'] as $index => $packer): ?>
                                    <?php 
                                        // Determine performance status based on SKU Diversity Index
                                        $avg_sku = $packer['avg_sku_per_resi'] ?? 0;
                                        $total_qty = $packer['total_qty'] ?? 0;
...
$(document).off('submit', '#form-filter-kpi-packer').on('submit', '#form-filter-kpi-packer', function(e) {
    ...
    function proceedWithSubmission() {
        ...
        var reportrange = window.kpiPackerReportRange;
...
function createRankingChart() {
        ...
        <?php if (isset($dashboard_stats['top_packers_inti']) && count($dashboard_stats['top_packers_inti']) > 0): ?>
            <?php foreach ($dashboard_stats['top_packers_inti'] as $packer): ?>
                allPackers.push({
                    ...
                    total_resi: <?= $packer['total_resi'] ?>,
                    total_qty: <?= $packer['total_qty'] ?>,
                    total_sku_unique: <?= $packer['total_sku_unique'] ?>,
                    avg_sku_per_resi: <?= $packer['avg_sku_per_resi'] ?>,
                    tim: 'INTI'
                });
            <?php endforeach; ?>
        <?php endif; ?>
        ...
        // Calculate performance score for each packer
        allPackers.forEach(function(packer) {
            var sku_diversity = packer.total_qty > 0 ? packer.total_sku_unique / packer.total_qty : 0;
            
            // Performance score calculation (0-100)
            var volume_score = Math.min(packer.avg_sku_per_resi / 5, 1) * 50; // 50% volume
            var diversity_score = sku_diversity * 50; // 50% diversity
            packer.performance_score = Math.round(volume_score + diversity_score);
        });
                        ...
                        tooltip: {
                            callbacks: {
                                afterLabel: function(context) {
                                    ...
                                    return [
                                        'Score: ' + packer.performance_score + '/100',
                                        'Total Resi: ' + packer.total_resi,
                                        'Avg SKU/Resi: ' + packer.avg_sku_per_resi.toFixed(2),
                                        'Diversity: ' + Math.round((packer.total_sku_unique / packer.total_qty) * 100) + '%',
                                        'Tim: ' + packer.tim
                                    ];
                                }
                            }

$(function() {
        ...
        return {
            ...
            formSubmit: function(form) {
                ...
                $.ajax({
                    ...
                    data: new FormData(form.context), // Data sent to server, a set of key/value pairs (i.e. form fields and values)
    ...
    var formElements = function() {
        ...
        var feRandomPassword = function() {
            $('#btn_generate_password').on('click', function() {
                $('input[name=password]').val(Math.random().toString(36).slice(-6));
            });
        }
    ...
    var uiElements = function() {
        ...
        var uiSummernote = function() {
            ...
            /* Long summernote editor */
            if ($(".summernote_long").length > 0) {
                $(".summernote_long").summernote({
                    height: 500,
                    codemirror: {
                        mode: 'text/html',
                        htmlMode: true,
    ...
    var jsTree = function() {
        ...
        var jstree = function() {
            ...
            if (typeof attr !== typeof undefined && attr !== false) {
                ...
                $('#menu_tree').closest('form').submit(function(e) {
                    jsTreeInstance = $('#menu_tree').jstree(true);
                    selectedNodes = jsTreeInstance.get_selected();

                    $('#jstree_input').empty();

                    // init new jstree obj
                    jsTreeInstanceParent = $('#menu_tree').jstree();
                    selectedNodesParent = jsTreeInstanceParent.get_selected();

                    // check has parents?
                    selectedNodesParent.forEach(function(nodeId) {
                        var i = nodeId;
                        while (jsTreeInstanceParent.get_parent(i) != "#") {
                            selectedNodes.push(jsTreeInstanceParent.get_parent(i));
                            i = jsTreeInstanceParent.get_parent(i);
                        }
                    });

                    selectedNodes = selectedNodes.filter(function(value, index, self) {
